...
int main_()
{
  ...
  // The problem with above code is, every time when we make car, we need to intialize the engine also
  using namespace boost;
  auto injector = di::make_injector(
    di::bind<ILogger>().to<ConsoleLogger>()
  );
  
  //While creating car through injector, it will intialize the dependency also, here is the engine
  //it will now through constructor
