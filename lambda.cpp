...
// It is an expression that represent doing something
//
...
void lambda_main()
{
    ...
    // Print with out lambda
    for_each(v.begin(), v.end(), print);

    // Syntax
    // [lambda expression](input parameter) -> return type {body}
    
    // Use lambda to print the no
    for_each(v.begin(), v.end(), [](int n) {cout << n << " "; });
    cout << endl;
    std::cout << std::endl << "-----------------------" << std::endl;

    //lambda can be written in multiple lines
    for_each(v.begin(), 
             v.end(), 
             [](int n) 
                { 
